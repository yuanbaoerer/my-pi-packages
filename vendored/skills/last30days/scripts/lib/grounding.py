...
def tavily_search(
    ...
    for i, r in enumerate((data.get("results", []))[:count]):
        ...
        pub_date = _normalize_date(raw_date[:10]) if raw_date else None
        # Tavily doesn't always return dates; treat undated results as recent
    ...
    artifact = {"label": "tavily", "webSearchQueries": [query], "resultCount": len(items)}
    return items, artifact
...
def web_search(
    ...
    if backend == "tavily":
        ...
        if not key:
            raise RuntimeError("TAVILY_API_KEY is required when web_backend='tavily'")
        items, artifact = tavily_search(query, date_range, key)
    elif backend == "brave":
